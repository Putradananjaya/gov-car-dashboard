...
- **Kemampuan `aset.hapusPermanen` sudah dihapus** dari daftar (penghapusan permanen tidak ada lagi di aplikasi ini), diganti **`pengguna.hapus`** (hapus & pulihkan akun). Baris `role_permissions` lama untuk kemampuan yang sudah tidak ada dibiarkan saja — `getMatriks()` selalu membangun matriks dari `SEMUA_KEMAMPUAN`, jadi baris asing sekadar diabaikan.
...
### Kebijakan penghapusan: soft delete di seluruh aplikasi

**Tidak ada satu pun fitur di aplikasi ini yang benar-benar membuang baris data.** Setiap "hapus" hanya menyembunyikan baris dari pembacaan; isinya tetap tinggal di Postgres.

- **Mekanismenya** `@DeleteDateColumn` bawaan TypeORM di kolom `dihapus_pada` (`timestamptz`, nullable), lewat kelas basis `EntitasSoftDelete` di `server/src/common/soft-delete.ts`. Dipakai oleh `users`, `vehicle_operational`, `loans`, `loan_documents`, `service_records`, `vehicle_photos`.
- **Kenapa `@DeleteDateColumn`, bukan kolom penanda biasa**: TypeORM otomatis menambahkan `WHERE dihapus_pada IS NULL` ke **setiap** `find*()` dan QueryBuilder entity tersebut (lihat `QueryBuilder.createWhereExpression`). Jadi tidak ada jalur baca yang bisa lupa menyaring, dan frontend tidak perlu tahu apa-apa: data terhapus memang tidak pernah terkirim. Untuk sengaja melihatnya, pakai `withDeleted: true`.
- **`vehicle_assets` adalah perkecualian yang sengaja**: penandanya kolom `dihapus_pada` bertipe `varchar` ISO miliknya sendiri, karena nilainya **ikut terkirim di DTO** dan dipakai frontend (jalur impor menyimpannya sebagai bagian dari snapshot pembatalan, sehingga NIBAR yang pernah dihapus tetap terhitung "pembaruan" bukan "aset baru"). Penyaringannya karena itu di sisi tampilan — `aset-list`, `aset-detail`, ketiga dasbor, `peminjaman-form`, dan adapter `CarCompatRepository` (Inventaris/Monitoring GPS) semuanya membuang baris yang `dihapusPada`-nya terisi. Mengubah kolom ini jadi `@DeleteDateColumn` berarti `ALTER COLUMN ... TYPE timestamptz` atas data varchar yang sudah ada — gagal di boot tanpa klausa `USING`, jadi tidak dilakukan.
- **`audit_logs` tidak pernah dihapus sama sekali** (append-only), dan penghapusan di `refresh_tokens` / `login_attempts` tetap `DELETE` sungguhan — keduanya pembersihan keamanan, bukan data pengguna.
- **`upsert()` memulihkan**: tiap service memanggil `pulihkanBilaPernahDihapus()` sebelum `save()`. `save()` sendiri sudah aman terhadap primary key (pencarian INSERT-atau-UPDATE-nya memakai `withDeleted: true`), yang **tidak** dilakukannya adalah mencabut penanda — tanpa langkah ini baris tersimpan dengan isi baru tapi tetap tak terbaca siapa pun.
- **Tidak ada tabel baru** untuk ini. Satu kolom nullable per tabel sudah cukup, dan pertanyaan "siapa menghapus apa, kapan, dengan alasan apa" sudah dijawab `audit_logs` (aksi `hapus`, `hapus-pengguna`, `pulihkan-pengguna`) — tabel arsip terpisah hanya akan menduplikasinya.
- **Yang belum ada layar pemulihannya**: hanya pengguna yang punya UI arsip + tombol pulihkan (tab "Pengguna Terhapus"). Aset, peminjaman, dokumen, foto, dan riwayat servis pemulihannya masih lewat SQL (`UPDATE ... SET dihapus_pada = NULL`).

| Tabel | PK | Kolom kunci | Catatan |
|---|---|---|---|
| `users` | `id` (varchar) | `nip` (unique), `nama`, `jabatan`, `unitKerja`, `peran`, `aktif`, `passwordHash` (bcrypt), `terakhirMasuk`, `dihapusPada` (soft delete) | Peran: superadmin/admin/pegawai/pejabat_penatausahaan/pimpinan (5 sejak Fase 6). `aktif` dan `dihapusPada` beda maksud: akun **nonaktif** masih terlihat di daftar (mis. cuti panjang), akun **terhapus** tidak terlihat sama sekali dan tidak bisa masuk. NIP akun terhapus tetap terpakai (kolom unique) — `create()` menolaknya dengan pesan yang menunjuk ke arsip. Kolom `peran` tetap `varchar` biasa (bukan Postgres ENUM), jadi menambah nilai baru tidak perlu perubahan skema |
...
| `vehicle_operational` | `nibar` | `kondisi` ('Baik'/'Rusak Ringan'/'Rusak Berat'), `status` ('Tersedia'/'Dipinjam'/'Servis'/'Tidak Layak'), `telemetri` (jsonb: lat/lng/kecepatan/levelBbm/sumber/waktu), `catatan`, `dihapusPada` (soft delete) | 1:1 dengan `vehicle_assets` lewat `nibar` |
| `loans` | `id` | `nibar`, `pemohonId`, `pemohon` (jsonb snapshot: nama/nip/jabatan/unitKerja/noHp — **disalin**, bukan join, supaya histori tidak berubah kalau data user berubah), `statusPermohonan`, `jenisPermohonan`, `rute` (baru, Fase 6), `status` (workflow **2 tahap**: Draft→Diajukan→**Disetujui** (tahap 1, cek ketersediaan)→**Berjalan** (tahap 2, serah terima)→Selesai, atau Ditolak dari Diajukan/Disetujui), `tingkatUrgensi`, `odometerKeluar/Masuk`, `bbmKeluar/Masuk` (baru), `kondisiKeluar/Masuk` + `catatanKondisiKeluar/Masuk` (baru, enum sama seperti `KondisiAset` di `vehicle_operational`, diduplikasi bukan di-reference), `kunciDiserahkanPada/Dikembalikan Pada` (baru, timestamp ISO, wajib dicentang di modal serah-terima/kembalikan sebelum submit) | Form peminjaman kendaraan, field-nya mengikuti form resmi "FRM-01" (disebut di komentar). Field baru Fase 6 semuanya nullable — aman ditambah lewat `synchronize: true` tanpa migration manual. Plus `dihapusPada` (soft delete): peminjaman yang dihapus otomatis berhenti memblokir jadwal kendaraan, karena `assertNoScheduleConflict` memakai QueryBuilder yang ikut menyaringnya. |
| `loan_documents` | `id` | `loanId`, `kind` ('utama'/'lain'), `fileName`, `mimeType`, `size`, **`blob` (bytea)**, `dihapusPada` (soft delete) | Dokumen surat tugas dkk disimpan **langsung di Postgres sebagai binary**, bukan filesystem/S3. Menghapus dokumen tidak membuang `blob`-nya — lampiran surat resmi, jadi **ukuran DB tidak menyusut** saat dokumen dihapus |
| `service_records` | `id` | `nibar`, `tahun`, `uraian`, `odometerKm`, `biaya`, `sumber` ('impor'/'input-manual'), `dihapusPada` (soft delete) | Riwayat servis & histori pajak |
| `vehicle_photos` | `nibar` | `mimeType`, **`blob` (bytea)**, `dihapusPada` (soft delete) | Foto kendaraan, juga disimpan sebagai binary di Postgres. Sama seperti dokumen: dihapus ≠ `blob`-nya hilang |
| `audit_logs` | `id` | `waktu`, `pelakuId`, `pelakuNama`, `aksi`, `entitas`, `entitasId`, `nilaiLama`/`nilaiBaru` (jsonb) | Log audit generik untuk semua entitas. **Append-only** — tidak punya kolom soft delete karena tidak pernah dihapus |
...
| `GET/POST/PUT/:id /users` | izin `pengguna.kelola` untuk create/update/reset-password | `GET` daftar bisa semua role login, dan **hanya akun yang belum dihapus** |
| `POST /users/:id/reset-password` | izin `pengguna.kelola` | |
| `GET /users/terhapus` | izin `pengguna.hapus` | Arsip akun terhapus (`withDeleted`), untuk tab "Pengguna Terhapus" |
| `DELETE /users/:id` | izin `pengguna.hapus` | **Soft delete.** Ditolak kalau targetnya diri sendiri, atau superadmin aktif terakhir |
| `POST /users/:id/pulihkan` | izin `pengguna.hapus` | Kebalikan DELETE — akun kembali terbaca |
| `GET, GET/:nibar, PUT/:nibar /vehicle-assets` | login saja (tidak ada role check!) | |
| `DELETE /vehicle-assets/:nibar` | izin `aset.hapus` / `aset.impor` | **Bukan lagi hapus permanen** — diarahkan ke `softDelete()` yang sama. Dipertahankan demi klien lama & jalur pembatalan impor |
| `POST /vehicle-assets/:nibar/soft-delete` | izin `aset.hapus` | Isi `dihapusPada` |
| `GET, GET/:nibar, PUT/:nibar, DELETE/:nibar /vehicle-operational` | PUT/DELETE: izin aset (DELETE butuh `aset.hapus`/`aset.impor`) | `DELETE` = soft delete |
| `GET, GET/:id, PUT/:id, DELETE/:id /loans` | login saja (lihat baris khusus di bawah untuk transisi status); `DELETE` = soft delete | `PUT/:id` generik **ditolak** (400) kalau `status` di body termasuk `Disetujui/Berjalan/Ditolak/Selesai` — dipaksa lewat endpoint aksi khusus |
...
| `GET, PUT/:nibar, DELETE/:nibar /photos` | PUT/DELETE: izin `aset.ubah`/`aset.impor` | `DELETE` = soft delete; `blob` tetap tersimpan |
| `GET, PUT/:id, DELETE/:id /service-records` | izin `servis.input` | `DELETE` = soft delete |
...
- **`pengguna-list`** — manajemen user (superadmin only). **Fase 6**: dropdown peran sekarang 5 opsi dengan label human-readable (`peranLabel()`), daftar Unit Kerja/OPD ditambah 7 Bidang di bawah Badan Keuangan, Pendapatan dan Aset Daerah. Punya **tiga tab**: "Daftar Pengguna", "Pengguna Terhapus" (arsip + tombol pulihkan, izin `pengguna.hapus`), dan "Hak Akses Peran". Arsip **tidak** ikut dimuat saat halaman dibuka — `muatTerhapus()` baru dipanggil saat tabnya dipilih, jadi `DataSyncService` tidak menariknya tiap putaran.
